...
# Recode this entire notebook. 

Look to 61 recoding to see exactly what fields need to be brought forward. 

Ignore the factored files. 

Keep the lengthening and combining code.
...
```{r Define lengthen() for 16/17 df}
...
<!-- ```{r Define lengthen_16factor() for 16/17 df} -->
...
# Creating a list of unwanted variables
...
```{r Define unmerged()}
unmerged <- function(unfactor, unfactor16){
  #Lengthens all dataframes through above wrtten lengthen functions
  unfactor <- lengthen(unfactor)
  
  # pais in the unfactor dataframe is character, in unfactor16 it's haven-labelled, change to character to enable the union_all
  unfactor16 <- unfactor16 %>%
...
merged <- function(unfactor){
  ...
  complete <- clean(unfactor)

  }
```


```{r Create clean df for each country}
#Argentina
argentina_clean <- unmerged(argentina_unique_unfactor, argentina_16_unfactor)

#Belize
belize_clean <- merged(belize_unique_unfactor)

#Bolivia
bolivia_clean <- unmerged(bolivia_unique_unfactor, bolivia_16_unfactor)

#Brazil
brazil_clean <- unmerged(brazil_unique_unfactor, brazil_16_unfactor)

#Canada
canada_clean <- unmerged(canada_unique_unfactor, canada_16_unfactor)

#Chile
chile_clean <- unmerged(chile_unique_unfactor, chile_16_unfactor)

#Colombia
#colombia_clean <- unmerged(colombia_unique_unfactor, colombia_16_unfactor)
#See Issue #11

#Costa Rica
costa_clean <- unmerged(costa_unique_unfactor, costa_16_unfactor)

#Ecuador
ecuador_clean <- unmerged(ecuador_unique_unfactor, ecuador_16_unfactor)

#El Salvador
elsalvador_clean <- unmerged(elsalvador_unique_unfactor, elsalvador_16_unfactor)

#Guatemala
guatemala_clean <- unmerged(guatemala_unique_unfactor, guatemala_16_unfactor)

#Guyana
guyana_clean <- unmerged(guyana_unique_unfactor, guyana_16_unfactor) 

#Honduras
honduras_clean <- unmerged(honduras_unique_unfactor, honduras_16_unfactor)

#Jamaica
jamaica_clean <- unmerged(jamaica_unique_unfactor, jamaica_16_unfactor)

#Mexico
mexico_clean <- unmerged(mexico_unique_unfactor, mexico_16_unfactor)

#Nicaragua
nicaragua_clean <- unmerged(nicaragua_unique_unfactor, nicaragua_16_unfactor)

#Panama
panama_clean <- unmerged(panama_unique_unfactor, panama_16_unfactor)

#Paraguay
paraguay_clean <- unmerged(paraguay_unique_unfactor, paraguay_16_unfactor)

#Peru
peru_clean <- unmerged(peru_unique_unfactor, peru_16_unfactor)

#Suriname
suriname_clean <- merged(suriname_unique_unfactor)

#United States
usa_clean <- unmerged(usa_unique_unfactor, usa_16_unfactor)

#Uruguay
uruguay_clean <- unmerged(uruguay_unique_unfactor, uruguay_16_unfactor)

#Venezuela
venezuela_clean <- unmerged(venezuela_unique_unfactor, venezuela_16_unfactor)
```


## Combining all clean versions
Making list of all the data frames
```{r Create clean-data list}
...
```{r Combine all clean-data together}
