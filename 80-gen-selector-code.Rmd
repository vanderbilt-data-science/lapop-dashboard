...
library(glue)
cat_numxl <- read_csv("lookups/questions_categories_v1-1.csv") 

measures_cat_short <- tibble(column_name = measures) %>%
  left_join(cat_numxl, by=("column_name")) %>%
  select(column_name, category_short_es, question_short_es, question_es) %>% 
...
measures_cat_short %>% 
  arrange(category_short_es, question_short_es) %>% 
  glue_data('{column_name}, {category_short_es}:{question_short_es}')
```
...
```{r calc qtext, echo=FALSE}
cat("CASE [Choose Variable]
")

measures_cat_short %>% 
  glue_data("WHEN '{column_name}' THEN '{question_short_es}'")
...
```{r calc fullq, echo=FALSE}
cat("CASE [Choose Variable]
")

measures_cat_short %>% 
  glue_data("WHEN '{column_name}' THEN '{question_es}'")
...
label_val <- read_csv("lookups/values_labels_v1-1.csv")
```
...
label_val %>% 
  ...
  distinct(label_es, .keep_all = T) %>%
  summarise(lab_val = paste0(label_es, "--", value_recoded, collapse = "\n")) %>% 
  ungroup() %>%
