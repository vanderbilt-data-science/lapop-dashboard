...
total_long_common_recoded <- final_data_long %>%
  ...
  mutate(answer_dimension = if_else(column_name=="year"|is.na(label_es), #making sure that in the dropdown menu, year doesn't have too many leading spaces,
                                    label_es,
                                    paste0(answer_measure,":", paste(rep(" ",100),collapse=""), label_es)),
...
total_long_common_narrow <- total_long_common_recoded %>% 
  ...
  select(country, person_id, weight1500, column_name, category_question, category_colname, question, column_name, answer_measure, answer_dimension) 
```
...
write_csv(final_wide, "lapop_wide_v0-13-4.csv", na = "NULL ") 
