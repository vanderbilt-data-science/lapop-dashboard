...
get_country <- function(file, filename){
  ...
  if("pais" %in% colnames(file)){
    ...
    print(file$pais[[1]])
    print(country_code)
    stopifnot(substr(filename,1,3)==country_code)
    print("Successful!")
    return(file)
  }
  else{
...
get_year <- function(file, filename){
  ...
  if("year" %in% colnames(file)){
    ...
    stopifnot(substr(filename,5,8)==year_in_file)
    print("Successful!")
    return(file)
  }
  else{
...
making_uniqueID <- function(country) {
  ...
  country <- subset(country, select = -c(value_recoded)) %>%
    ...
    #Set idnum to row_number
    mutate(., idnum = row_number()) %>%
    #Makes new column, person_id
    mutate(.,
           person_id = paste(paste( 
             #Takes first 3 letters of country name
             value_recoded,
